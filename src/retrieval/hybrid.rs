...
use crate::retrieval::dense_route::{search_dense_route, search_local_hash_route};
use crate::retrieval::fact_route::search_fact_route;
use crate::retrieval::fts_route::search_fts_route;
use crate::retrieval::fusion::{ChunkRoute, empty_retrieval_trace, merge_chunk_routes};
...
pub(crate) fn search_chunks_with_trace(
    ...
    let embedding_rows = search_local_hash_route(storage, author, query, 30)?;
...
pub(crate) fn search_chunks_with_dense_vector_trace(
    ...
    let mut ranked_routes = base_routes(storage, author, query, &terms)?;
    let dense_rows = search_dense_route(storage, author, model, model_version, query_vector, 30)?;
...
fn add_profile_route(
    ...
    if !profile_rows.is_empty() {
        ranked_routes.push(("profile", profile_rows));
    }
    Ok(())
}
